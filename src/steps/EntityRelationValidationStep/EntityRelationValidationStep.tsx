...
export type EntityRelationValidationStepProps<T extends string> = {
  data: RawData[]
...
export const EntityRelationValidationStep = <T extends string>({
  data,
  headerValues,
  onContinue,
  onBack,
}: EntityRelationValidationStepProps<T>) => {
  ...
  const handleOnContinue = useCallback(async () => {
    if (unmatchedRequiredFields.length > 0) {
      ...
      setIsLoading(true)
      await onContinue(normalizeTableData(columns, data, fields), data.slice(0))
      setIsLoading(false)
    }
